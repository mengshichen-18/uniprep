...
SESSION_PREFIX="${SESSION_PREFIX:-featgen0428rep}"

BUILD_CONTEXTS="${BUILD_CONTEXTS:-0}"
CONTEXTS_DIR="${CONTEXTS_DIR:-${REPO_DIR}/outputs/featgen_contexts}"
TOP_ATOMS="${TOP_ATOMS:-8}"
TRAIN_LOG_DIR="${TRAIN_LOG_DIR:-${REPO_DIR}/outputs/tmux_logs/featgen_replicates}"
ARTIFACT_ROOT="${ARTIFACT_ROOT:-${REPO_DIR}/outputs/featgen_replicates}"
...
if [[ "${#GPUS[@]}" -lt 2 ]]; then
  echo "[ERROR] launch_featgen_replicates_tmux.sh expects at least two GPU ids" >&2
  exit 2
fi
...
for seed in "${SEEDS[@]}"; do
  for task in "${TASKS[@]}"; do
    for dataset in "${DATASETS[@]}"; do
      ...
      printf "%s\t%s\t%s\t%s\n" "${weight}" "${seed}" "${task}" "${dataset}" >> "${jobs_tmp}"
    done
  done
done

while IFS=$'\t' read -r _weight seed task dataset; do
  best_gpu=""
  best_load=""
  for gpu in "${GPUS[@]}"; do
    load="${GPU_LOADS[${gpu}]}"
  ...
  run_tag="${SESSION_PREFIX}_s${seed}_${short}_${dataset}"
  job_log="${TRAIN_LOG_DIR}/${run_tag}.log"
  artifact_dir="${ARTIFACT_ROOT}/seed${seed}/${task}/${dataset}"
  queue_script="${QUEUE_SCRIPTS[${best_gpu}]}"

  cat >> "${queue_script}" <<EOF
echo "[START] seed=${seed} task=${task} dataset=${dataset} gpu=${best_gpu} run_tag=${run_tag}"
...
echo "[DONE] seed=${seed} task=${task} dataset=${dataset} gpu=${best_gpu} run_tag=${run_tag}"
EOF

  GPU_LOADS["${best_gpu}"]=$(( GPU_LOADS["${best_gpu}"] + _weight ))
...
for gpu in "${GPUS[@]}"; do
  ...
  echo "[START] session=${session} gpu=${gpu} queue=${queue_script} log=${launcher_log} assigned_weight=${GPU_LOADS[${gpu}]}"
done

echo "[DONE] launched replicate queues with prefix=${SESSION_PREFIX}"
